...
uint8_t start_speed = 1;  // currently required to be uint8_t due to u8g2 var
                          // function, to be changed to allow higher values
...
void IRAM_ATTR ISR_MENU() { debounce_tmr = (millis() + 20); }
  ...
  switch (state) {
    case Accel:
        ...
        if (acc_started) {
          ...
          if ((int)(ground_speed * units_mult) >= end_speed) {
            ...
            acc_started = 0;
                   ...
                   ((int)(ground_speed * units_mult) >= start_speed)) {
      ...
      if (((millis() - last_disp_refresh) >= (1000 / DISPLAY_HZ)) &&
        ...
        print_val = Interp(prev_speed, ground_speed, interp_count, INTERP_NUM) *
                    units_mult;
        display.UpdateDispAccel(print_val, pvt.num_sv, ((float)timer / 1000),
                                units_str);
      } else if (error == 1) {
    ...
    case Speed:
      ...
      if (((millis() - last_disp_refresh) >= (1000 / DISPLAY_HZ)) &&
        ...
        print_val = Interp(prev_speed, ground_speed, interp_count, INTERP_NUM) *
                    units_mult;
        display.UpdateDispSpeed(print_val, pvt.num_sv, (max_speed * units_mult),
                                units_str);
      } else if (error == 1) {
        display.ErrorMsg(error_msg.c_str());
      }
    ...
    case Menu:
      ...
      debounce_tmr = 0;
      switch (disp->userInterfaceSelectionList(
          "Settings", 1,
          "Change Mode\nChange Start Speed\nChange End Speed\nChange "
          "Distance\nChange Units\nExit")) {
        case 1:
          ...
          break;
        case 2:
          disp->userInterfaceInputValue("Change Start Speed", "", &start_speed,
                                        0, 244, 3, units_str);
          break;
        case 3:
          disp->userInterfaceInputValue("Change End Speed", "", &end_speed, 0,
                                        244, 3, units_str);
          break;
        case 5:
          switch (disp->userInterfaceSelectionList("Select Units", 1,
                                                   "km/h\nmph\nm/s")) {
            case 1:
              units_str = unit_kmh;
              units_mult = 0.0036;
    ...
    case ChangeMode:
      ...
      debounce_tmr = 0;
      disp->clearBuffer();
      state = disp->userInterfaceSelectionList(
                  "Select Mode:", 1, "Acceleration\nDistance\nTop Speed") -
              1;
      active_state = state;
      delay(BTN_DELAY);
      attachInterrupt(digitalPinToInterrupt(MENU_PIN), ISR_MENU, RISING);
