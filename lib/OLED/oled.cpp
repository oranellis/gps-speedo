...
void Display::UpdateDispAccel(float speed_inp, int sats, float timer,
                              char* units) {
  u8g2->clearBuffer();
...
void Display::UpdateDispSpeed(float speed_inp, int sats, float max_speed,
                              char* units) {
  u8g2->clearBuffer();
  u8g2->setFont(u8g2_font_logisoso46_tn);
  u8g2->setCursor(0, 49);
...
void Display::ErrorMsg(const char* message) {
  ...
  u8g2->drawButtonUTF8(64, 36, U8G2_BTN_INV | U8G2_BTN_BW2 | U8G2_BTN_HCENTER,
                       0, 2, 2, message);
  u8g2->sendBuffer();
}